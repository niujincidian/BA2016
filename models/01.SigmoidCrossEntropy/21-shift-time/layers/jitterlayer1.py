...
class JitterLayer(caffe.Layer):
	def setup(self, bottom, top):
		...
		self.shift_f = 1 # shift over frequency domain
		self.shift_t = 1 # shift over time domain
    ...
    def forward(self, bottom, top):
		...
		if ((not self.min_shift_t) and (not self.max_shift_t):
			top[0].data[...]=bottom[0].data[...] #don't shift over time domain
		else:
			if flag:
		        ...
		        if self.shift_t==0:
		        	...
		        	top[0].data[...] = shift(bottom[0].data, [0,0,0,self.shift_t])
		        	if self.shift_t>0:
